...
int main()
{
    ...
    while(t--)
    {
        ...
        for(ll i=1;i<=n;i++)
        {
            for(ll j=i+1;j<=n;j++)
            {
                ll x = dis(v[i].ff,v[i].ss,v[j].ff,v[j].ss);
                v2[x].pb({i,j});
            }
        }
        ll ans=0;
        for(auto it:v2)
        {
           ...
           {
               ll px1 = it.ss[i].ff ,py1 = it.ss[i].ss;
               for(ll j=i+1;j<it.ss.size();j++)
               {
                   ll px2 = it.ss[j].ff ,py2 = it.ss[j].ss;
                   if(px1==px2||py1==py2)
                   continue;
                   ans++;
               }
