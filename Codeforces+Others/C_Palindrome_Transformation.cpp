...
{
    ll x = min(abs(p - a.ff), abs(p - a.ss));
    ll y = min(abs(p - b.ff), abs(p - b.ss));
    return x <= y;
}
int main()
{
    ...
    while (t--)
    {
        ...
        {
            ll k1 = 0, k2 = 0 ,mpp=p, fp = 1, lp = (n / 2);
            if (p > (n / 2))
            {
                if (n % 2 == 0)
                    fp = lp + 1;
