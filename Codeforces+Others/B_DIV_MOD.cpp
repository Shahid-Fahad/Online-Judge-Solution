...
ll arr[100005], st1[400005], st2[400005];
void build(ll si, ll ss, ll se)
{
    ...
    st1[si] = (st1[2 * si] + st1[2 * si + 1]);
    st2[si] = (st2[2 * si] + st2[2 * si + 1]);
}
ll query(ll si, ll ss, ll se, ll qs, ll qe, ll qn)
{
    if (ss >= qs && qe >= se)
    {
        if (qn == 1)
    ...
    ll mid = (ss + se) / 2;
    return query(2 * si, ss, mid, qs, qe, qn) + query(2 * si + 1, mid + 1, se, qs, qe, qn);
}
