...
int main()
{
    ...
    while (t--)
    {
        ...
        ll n;
        cin >> n;
        ll arr[n + 5],arr2[n+5],pre[n+5]={0},suf[n+5]={0};
        for (ll i = 1; i <= n; i++)
            cin >> arr[i];
        for(ll i=2;i<=n-1;i++)
           arr2[i] = max(0LL,max(arr[i+1],arr[i-1])-arr[i]+1);
        arr2[1]=arr[1];
        arr2[n]=arr[n];
        ll ans1 = 0, ans2 = 0, ans3 = 1e18;
        for (ll i = 2; i <= n - 1; i += 2)
        {
            suf[i]=arr2[i]+suf[i-2];
            ans1+=arr2[i];
        }
        for (ll i = n - 1; i >= 2; i -= 2)
        {
            pre[i]=arr2[i]+pre[i+2];
            ans2+=arr2[i];
        }
        if (n % 2 == 0)
        {
