...
ll n;
ll dp[(1 << 11) + 5][12];
pair<ll, ll> arr[12][12];
ll fn(ll mask,ll pos, ll out)
{
    if (mask == (1 << n - 1))
        return 0;
    if (dp[mask][pos] != -1)
    ...
    for (ll i = 0; i < n; i++)
    {
        ...
        if (!f)
        {
            ret = max(ret, fn(mask|(1<<i),pos,arr[pos][i].ss + 1) + arr[pos][i].ff);
            ret = max(ret, fn(mask|(1<<pos),i,arr[i][pos].ss + 1) + arr[i][pos].ff);
        }
    }
