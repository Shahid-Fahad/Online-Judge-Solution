...
void build(ll si, ll ss, ll se)
{
    ...
    if (st[2 * si].mn < st[2 * si + 1].mn)
        st[si] = st[2 * si];
    else if (st[2 * si].mn > st[2 * si + 1].mn)
        st[si] = st[2 * si + 1];
    else
      st[si] = {st[2 * si + 1].mn, st[2 * si].fr + st[2*si+1].fr};
}
void query(ll si, ll ss, ll se, ll qs, ll qe)
{
    if (se < qs || qe < ss)
        return;
    if (qs <= ss && qe >= se)
