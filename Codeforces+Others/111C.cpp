...
int dp[1000005][10];
ll f(ll dignum, ll start)
{
    if (dignum == 1)
        return 1;

    if (dp[dignum][start] != -1)
        return dp[dignum][start];
    ll a = 0;
    for (ll j = max(1LL, start - 1); j <= min(9LL, start + 1); j++)
    {
        a = modAdd(f(dignum - 1, j), a);
    }

 return dp[dignum][start] = a;
}
int main()
{
