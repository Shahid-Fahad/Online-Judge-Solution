...
ll f(ll pos, ll rem, ll sum)
{
    ...
    if (dp[pos][sum] != -1)
        return dp[pos][sum];
    ll ans = 0;
    for (ll i = pos, j = 1; j <= rem; j++, i++)
    {
        i = (i % n);
        if (arr[i] == 0)
            continue;
        if (rem >= arr[i])
            ans = max(ans, f((pos + arr[i]) % n, rem - arr[i], modAdd(sum, arr[i])));
    }
    return dp[pos][sum] = ans;
}
ll f2(ll pos, ll rem, ll sum)
{
    if(pos>n)
    return 0;
    if (sum == xt)
    ...
    if (dp[pos][sum] != -1)
        return dp[pos][sum];
    ll ans = 0;
    ans = f2(pos + 1, rem, sum);
    if (arr[pos] >0 && rem >= arr[pos])
    {
        ans = modAdd(ans, f2((pos + arr[pos]) % n, rem - arr[pos], sum + arr[pos]));
    }
    return dp[pos][sum] = ans;
}
int main()
