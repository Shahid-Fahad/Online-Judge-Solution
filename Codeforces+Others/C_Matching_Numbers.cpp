...
int main()
{
    ...
    while (t--)
    {
        ...
        //         vp.pb({1,2*n});
        //         ll k = (n/2);
        //         for(ll j=i-1,x=k,sm=(1+2*n)+1;x>0;x--,sm++,j--)
        //         {
        //             if(j!=(sm-j))
        //             vp.pb({j,sm-j});
        //         }
        //         for(ll j=i-k-1,x=k,sm=(1+2*n)-k;x>0;x--,sm++,j--)
        //         {
        //             if(j!=(sm-j))
        //             vp.pb({j,sm-j});
        //         }
        ...
        vp.pb({1, 2 * n});
        ll k = (n / 2);
        for (ll j = 2*n - 1, x = k, sm = (1 + 2 * n) + 1; x > 0; x--, sm++, j--)
        {
           
            if (j != (sm - j))
                vp.pb({j, sm - j});
        }
        for (ll j = 2*n - k - 1, x = k, sm = (1 + 2 * n) - k; x > 0; x--, sm++, j--)
        {
