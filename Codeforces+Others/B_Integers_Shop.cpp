...
int main()
{
    ...
    while(t--)
    {
        ...
        ll lw=v[0].ff.ff,hw=v[0].ff.ss,sm=0;
        map<ll,ll>mpl,mph;
        ll ans=v[0].ss;
        for(ll i=0;i<v.size();i++)
        {
            ll x=v[i].ff.ff,y=v[i].ff.ss,z=v[i].ss;

            if(mpx.find({x,y})!=mpx.end())
            mpx[{x,y}] = min(mpx[{x,y}],z);
            else
            mpx[{x,y}]=z;
