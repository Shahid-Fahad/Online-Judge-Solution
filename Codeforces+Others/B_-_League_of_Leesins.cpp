...
int main()
{
    ...
    while (t--)
    {
        ...
        for (ll i = 1; i <= n - 2; i++)
        {
            ...
            mp[b]++;
            mp[c]++;
            tr[{a, b}].pb(c);tr[{b, a}].pb(c);
            tr[{b, c}].pb(a); tr[{c, b}].pb(a);
            tr[{a, c}].pb(b); tr[{c, a}].pb(b);
        }
        vector<ll> f, s;
        for (auto it : mp)
        {
        ...
        for (ll i = 0; i < f.size(); i++)
        {
            ...
            {
               // cout << "YEST " << f[i] << " " << s[j] << endl;
                if (tr[{f[i], s[j]}] .size()!=0)
                {
                    int ans[n + 5];
                    fr = f[i];
                    sc = s[j];
                    ...
                    for (ll k = 3; k <= n; k++)
                    {
                       // cout << "ASHO " << fr << " " << sc << endl;
                        used.insert(fr);
                        used.insert(sc);
                        //ll val=tr[{fr, sc}];
                        ll val;
                        for(auto it:tr[{fr,sc}])
                        {
