...
int main()
{
    ...
    while (t--)
    {
        ...
        for (ll i = n - 1; i >= 0; i--)
        {
            ll lim = tm[i] - hl[i] + 1;
            ll fl = 0;
            for (ll j = i - 1; j >= 0 && tm[j] >= lim; j--)
            {
                ll val = hl[i] - (tm[i] - tm[j]);
                if (hl[j] > val)
                {
                    for (ll k = j + 1; k<n&&tm[k]-hl[k]+1<=tm[j]; k++)
                    {
                ...
                {
                    for (ll k = j; k >= 0 && tm[k] >= lim; k--)
                    {
                        ll p = hl[i] - (tm[i] - tm[k]);
                        if (hl[k] > p)
                        {
                            for (ll f = k + 1; f<n&&tm[f]-hl[f]+1<=tm[k]; f++)
                            {
                                ll p2 = hl[k] + (tm[f] - tm[k]);
                                hl[f] = p2;
                            }
                        }
                        else
