...
string x, y, k;
ll dp[11][2][2][92];
ll f(ll pos, bool iss, ll zs, bool ill)
{
    if (pos == k.size())
        return zs;
    if (dp[pos][iss][ill][zs] != -1)
        return dp[pos][iss][ill][zs];
    ll lo = 0, hi = k[pos] - '0';
    if (iss)
        hi = 9;
    ll ans = 0;
    for (ll i = lo; i <= hi; i++)
    {
        bool newis = iss | (i < hi);
        bool newill = ill | i;
        ans += f(pos + 1, newis, zs + ((i==0)&&ill), (ill|i));
    }
    return dp[pos][iss][ill][zs] = ans;
}
int main()
{
