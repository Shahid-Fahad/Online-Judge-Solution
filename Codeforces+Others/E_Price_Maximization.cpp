...
int main()
{
    ...
    while (t--)
    {
        ...
        for (ll i = k - 1; i >= 0; i--)
        {
            for (ll j = k - i; j <= i; j++)
            {
                //cout << "TET " << i << " " << j << " " << ans << endl;
                while (mp[i].size() > 0 && mp[j].size() > 0)
                {
                    //cout << "COMING " << mp[i].back() << " " << mp[j].back() << endl;
                    if (i == j)
                    {
                        if (mp[i].size() < 2)
                            break;
    ...
    //     //cout << "ET " << ans << endl;
    //     for (ll i = mp[0].size() - 2; i >= 0; i -= 2)
    //     {
    //         ans += ((mp[0][i] + mp[0][i + 1]) / k);
    //     }
    //    // cout << "PT " << ans << endl;
    //     if (mp[0].size() % 2 == 1)
    //     {
