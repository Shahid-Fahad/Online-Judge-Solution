...
ld arr[100005];
ll dp[100005][3];
ld f1(ll i, ll c, ll s, ll sk)
{
    if (i > n)
    {
        ld r = ((s+0.0) / (c + 0.0));
        cout << "TETS " << s << " " << c  << " " << r << endl;
        return r;
    }
    if (dp[c][sk] != -1)
        return dp[c][sk];
    ld a = -1e18, b = -1e18, b2 = -1e18;
    if (sk == 0)
    {
        a = f1(i + 2, c + 1, s + arr[i], 0);
        if (i + 1 <= n)
        b = f1(i + 2, c + 1, s + arr[i + 1], 0);
        b2 = f1(i + 1, c, s, 1);
    }
    else
    {
