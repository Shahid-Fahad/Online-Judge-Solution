...
map<string,ll>oc;
bool cus(pair<string, pair<ll, ll>> a, pair<string, pair<ll, ll>> b)
{
    if(a.ff.size()==b.ff.size())
    return oc[a.ff]>oc[b.ff];
    else
...
int main()
{
    ...
    while (t--)
    {
        ...
        vector<pair<string, pair<ll, ll>>> vv;
        ...
        for (auto it : vv)
        {
            if (mp[it.ff] == 1)
            {
                ...
                ll cn = 0;
                for (ll i = it.ss.ff; i <= it.ss.ss; i++)
                {
                    if (s[i] != 'C')
                    {
                        cn++;
                ...
                if (cn)
                {
                    kk.pb({it.ss.ff + 1, it.ss.ss + 1});
                    for (ll i = it.ss.ff; i <= it.ss.ss; i++)
                        s[i] = 'C';
        cout << "PDPD " << it.ff << " " << s << endl;
                }
            }
        }
        ll cn = 0;
