...
int arr[105];
int dp[105][1000005];
ll f(ll ind,ll num,vector<int>& coins)
{
    if(num==0)return 1;
    if(ind<0)return 0;
    if(dp[ind][num]!=-1)return dp[ind][num];
    ll ways=0;
    for(ll cs=0;cs<=num;cs+=coins[ind])
    {
        ways+=(f(ind-1,num-cs,coins));
    }
    return dp[ind][num]=ways;
}
// int change(int amount, vector<int>& coins) {
//     cout<< f(coins.size()-1,amount,coins) <<endl;
// }
int main()
{
    ...
    while(t--)
    {
        ...
        cout << f(coins.size()-1,k,coins) << endl;
    } 
}
