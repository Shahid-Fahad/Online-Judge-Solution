...
unordered_map<pair<ll,ll>,ll>block;
ll fn(ll x,ll y,ll cn,ll dir)
{
    if(cn==n)
    return 1;
    ...
    ll ans=0;
    if(!block[{x+a,y+b}])
    ans = modAdd(ans,fn(x+a,y+b,cn+1,x+y));
    if(!block[{x+c,y+d}])
    ans = modAdd(ans,fn(x+c,y+d,cn+1,x+y));
    if(!block[{x+e,y+f}])
    ans = modAdd(ans,fn(x+e,y+f,cn+1,x+y));
...
int main()
{
    ...
    while(t--)
    {
        ...
        for(auto it:dp)
        {
            ans = modAdd(ans,it.ss);
        }
        cout << fn(0,0,0,-1)  << endl;
    } 
}
