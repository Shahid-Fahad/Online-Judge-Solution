...
string x, y, z;
ll dp[2002][2][2002];
ll f(ll pos, bool iss, ll md)
{
    if (pos == z.size())
        return md == 0;
    if (dp[pos][iss][md] != -1)
        return dp[pos][iss][md];
    ll hi = z[pos] - '0';
    if (iss)
        hi = 9;
    ll ans = 0;
    if (pos % 2 == 1)
    {
        if (d <= hi)
        {
            ans = modAdd(ans, f(pos + 1,  iss|(d<hi) , ((md * 10) + d) % m));
        }
    }
    else
    {
        for (ll i = 0; i <= hi; i++)
        {
            if(i!=d)
            {
                ans = modAdd(ans, f(pos + 1, iss|(i<hi) , ((md * 10) + i) % m));
            }
        }
    }
    return dp[pos][iss][md] = ans;
}
int main()
{
    ...
    while (t--)
    {
        ...
        mem(dp, -1);
        z = y;
        ll a = f(0, 0, 0),it=x.size()-1;
        string kk = to_string(stoi(x));
        while(it>=0&&x[it]=='0')
        {
            x[it]='9';
            it--;
        ...
        x[it]--;
        mem(dp,-1);
        z=kk; /// z = kk dile output thikthak ashtese nah
        ll b = f(0,0,0);  
